        ...
        if cfg.student.pretrained_weights:
            ...
            if "model" in list(chkpt.keys()):
                ...
                if student_backbone_state_dict['pos_embed'].shape != chkpt['teacher']['backbone.pos_embed'].shape:
                    logger.info(f"OPTIONS -- pretrained weights without pos_embed: loading from {cfg.student.pretrained_weights}")
                    chkpt['teacher']['backbone.pos_embed'] = student_backbone_state_dict['pos_embed']
                self.student = nn.ModuleDict(student_model_dict)
                ...
                if student_backbone_state_dict['pos_embed'].shape != chkpt['pos_embed'].shape:
                    ...
                    logger.info(f"OPTIONS -- pretrained weights without pos_embed: loading from {cfg.student.pretrained_weights}")
                student_model_dict['backbone'].load_state_dict(chkpt, strict=True)
                self.student = nn.ModuleDict(student_model_dict)
