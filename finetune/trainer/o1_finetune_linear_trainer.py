'''
2024.8.28
TODO Full parameter fine-tuning, support multi-class CE, single-class BCE, multi-class BCE
'''
...
class O1FinetuneLinearTrainer(BaseTrainer):
    ...
    def get_model(self):
        ...
        self.model, self.embed_dim = build_o1_linear_finetune_model_from_cfg(cfg=self.args, only_teacher=True)
    ...
    def train(self, n_epoch):
        ...
        with tqdm(total=len(self.train_dataloader), desc=f"Train Epoch [{n_epoch}/{self.total_epoch}]", unit="batch") as pbar:
            ...
            for i, data_dict in enumerate(self.train_dataloader):
                ...
                # Swap -1 and 0
                if self.args.optim.o1_change01:
                    extra_features = extra_features + 1
                    extra_features[extra_features == 1] = 0
                    extra_features[extra_features == 2] = 1
                # Add random noise
                if self.args.optim.o1_label_smooth > 0:
                    # Generate random noise [-label_smooth, label_smooth]
                    random_noise = np.random.uniform(-self.args.optim.o1_label_smooth, self.args.optim.o1_label_smooth, size=extra_features.shape)
                    extra_features = extra_features + torch.tensor(random_noise)
                # Random mask
                if self.args.optim.o1_dropout > 0:
                    dropout_mask = torch.rand(extra_features.shape) > self.args.optim.o1_dropout
                ...
                images = merge_o1_input_tensors(images, extra_features)
...
class O1FinetuneSingleLinearTrainer(O1FinetuneLinearTrainer):
    def __init__(self, args):
        super().__init__(args)
    
    def get_model(self):
        # Full parameter fine-tuning
        self.model, self.embed_dim = build_o1_single_linear_finetune_model_from_cfg(cfg=self.args, only_teacher=True)