...
from models import build_linear_finetune_model_from_cfg, build_single_linear_finetune_model_from_cfg
import torch
import traceback
...
class FinetuneLinearTrainer(BaseTrainer):
    ...
    def get_model(self):
        self.model, self.embed_dim = build_linear_finetune_model_from_cfg(cfg=self.args, only_teacher=True)
...
class FinetuneSingleLinearTrainer(FinetuneLinearTrainer):
    def __init__(self, args):
        super().__init__(args)
    
    def get_model(self):
        self.model, self.embed_dim = build_single_linear_finetune_model_from_cfg(cfg=self.args, only_teacher=True)