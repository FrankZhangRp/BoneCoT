...
class ImageLatentsClassifier(nn.Module):
    def __init__(self, trained_model, latent_dim, num_classes, dropout_prob=0.3):
        ...
        self.to_visual_latent = nn.Linear(2097152, 512)
        
    def forward(self, *args, **kwargs):
        with torch.no_grad():
            kwargs['return_encoded_tokens'] = True
            image_latents = self.trained_model(*args, **kwargs)
            image_latents = image_latents.reshape(image_latents.shape[0], -1)
...
if __name__ == '__main__':
    ...
    dim_head = 32,
    heads = 8)
    model = ImageLatentsClassifier(base_model, 512, 18)
    checkpoint_path = '/data/dataserver01/zhangruipeng/code/BoneFM/model_preprocess/CT_LiPro.pt'
    # 我们这里只有visual transformer
    # checkpoint_path = '/data/dataserver01/zhangruipeng/code/BoneFM/model_preprocess/CT_CLIP_zeroshot.pt'
    ...
    model.load_state_dict(selected_model_dict, strict=False)
    
    model.cuda()
    # model = torch.nn.DataParallel(model)
    img = torch.randn(8, 1, 240, 480, 480).cuda()
    label = torch.randint(0, 18, (8,)).cuda()
    for i in range(100):
        print(img.shape)
        output = model(img)
